...
class WeatherAPI {
    ...
    private func baseURL() -> URL? {
        ...
        let baseURLString = "http://api.openweathermap.org/data/2.5/forecast/daily?"
                            + "lat=\(latitude)"
                            + "&lon=\(longitude)"
                            + "&cnt=16&mode=json&units=metric&appid=f9c4a2dfc491a99982b4a2d8888b8937"
    ...
    private func weatherResultWithData(data: Data?) -> WeatherResult {
        ...
        for weatherJSON in weatherInformation {
            if let weather = weatherWithCityJson(cityJson: cityInformation, weatherJson: weatherJSON) {
    ...
    private func weatherWithCityJson(cityJson: [String : AnyObject], weatherJson: [String: AnyObject]) -> Weather? {
